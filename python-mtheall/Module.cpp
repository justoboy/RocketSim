...
#define SET_TYPE_ATTR(type_, name_, value_)                                                                            \
...

	{
		...
		SET_TYPE_ATTR (RocketSim::Python::GameMode::Type,
		    "SPIKE_RUSH",
		    PyObjectRef::steal (PyLong_FromLong (static_cast<long> (RocketSim::GameMode::SPIKE_RUSH))));
		SET_TYPE_ATTR (RocketSim::Python::GameMode::Type,
		    "GRIDIRON",
		    PyObjectRef::steal (PyLong_FromLong (static_cast<long> (RocketSim::GameMode::GRIDIRON))));
...
#define ATTR_LIST(parent_, ns_, x_) SET_TYPE_ATTR (parent_.borrow (), #x_, BuildList (std::span (RocketSim::ns_::x_)))
		...
		{
			// SpikeRush
			auto spikeRush = createNamespace ();
			if (!spikeRush)
				return nullptr;

			ATTR_OBJECT (spikeRush, RLConst::SpikeRush, ACTIVATION_DELAY);
			ATTR_OBJECT (spikeRush, RLConst::SpikeRush, ATTACH_RADIUS);
			ATTR_OBJECT (spikeRush, RLConst::SpikeRush, MIN_ATTACH_TIME);
			ATTR_OBJECT (spikeRush, RLConst::SpikeRush, RELEASE_COOLDOWN);
			ATTR_OBJECT (rlConst, RLConst, ATTACH_INVULN_TIME);

			SET_TYPE_ATTR (rlConst.borrow (), "SpikeRush", spikeRush);
		}

		{
			// Gridiron
			auto gridiron = createNamespace ();
			if (!gridiron)
				return nullptr;

			ATTR_OBJECT (gridiron, RLConst::Gridiron, ROOF_LOCAL_OFFSET);
			ATTR_OBJECT (gridiron, RLConst::Gridiron, ATTACH_RADIUS);
			ATTR_OBJECT (gridiron, RLConst::Gridiron, REACQUIRE_COOLDOWN);
			ATTR_OBJECT (gridiron, RLConst::Gridiron, STEAL_INVULN_TIME);
			ATTR_OBJECT (gridiron, RLConst::Gridiron, WALL_FUMBLE_Z);
			ATTR_OBJECT (gridiron, RLConst::Gridiron, FOOTBALL_SEMI_MAJOR);
			ATTR_OBJECT (gridiron, RLConst::Gridiron, FOOTBALL_SEMI_MINOR);
			ATTR_OBJECT (gridiron, RLConst::Gridiron, FOOTBALL_HULL_RINGS);
			ATTR_OBJECT (gridiron, RLConst::Gridiron, FOOTBALL_HULL_RING_POINTS);
			ATTR_OBJECT (gridiron, RLConst::Gridiron, THROW_FUMBLE_UP);
			ATTR_OBJECT (gridiron, RLConst::Gridiron, THROW_FLIP_LOB_FWD);
			ATTR_OBJECT (gridiron, RLConst::Gridiron, THROW_FLIP_LOB_UP);
			ATTR_OBJECT (gridiron, RLConst::Gridiron, THROW_FLIP_LOB_SPIN);
			ATTR_OBJECT (gridiron, RLConst::Gridiron, THROW_DODGE_FWD);
			ATTR_OBJECT (gridiron, RLConst::Gridiron, THROW_DODGE_UP);
			ATTR_OBJECT (gridiron, RLConst::Gridiron, THROW_DODGE_SPIN);

			SET_TYPE_ATTR (rlConst.borrow (), "Gridiron", gridiron);
		}

		{
			// Dropshot
			auto dropshot = createNamespace ();
			if (!dropshot)
				return nullptr;

			ATTR_OBJECT (dropshot, RLConst::Dropshot, BALL_LAUNCH_Z_VEL);
			ATTR_OBJECT (dropshot, RLConst::Dropshot, BALL_LAUNCH_DELAY);
			ATTR_OBJECT (dropshot, RLConst::Dropshot, MIN_DOWNWARD_SPEED_TO_DAMAGE);
			ATTR_OBJECT (dropshot, RLConst::Dropshot, MIN_CHARGE_HIT_SPEED);
			ATTR_OBJECT (dropshot, RLConst::Dropshot, MIN_ABSORBED_FORCE_FOR_CHARGE);
			ATTR_OBJECT (dropshot, RLConst::Dropshot, MIN_ABSORBED_FORCE_FOR_SUPERCHARGE);
			ATTR_OBJECT (dropshot, RLConst::Dropshot, MIN_DAMAGE_INTERVAL);
			ATTR_OBJECT (dropshot, RLConst::Dropshot, NUM_TILES_PER_TEAM);
			ATTR_OBJECT (dropshot, RLConst::Dropshot, TEAM_AMOUNT);

			SET_TYPE_ATTR (rlConst.borrow (), "Dropshot", dropshot);
		}
		...
		ATTR_LIST (rlConst, RLConst, CAR_SPAWN_LOCATIONS_DROPSHOT);
		ATTR_LIST (rlConst, RLConst, CAR_SPAWN_LOCATIONS_GRIDIRON);
		ATTR_LIST (rlConst, RLConst, CAR_RESPAWN_LOCATIONS_SOCCAR);
		ATTR_LIST (rlConst, RLConst, CAR_RESPAWN_LOCATIONS_HOOPS);
		ATTR_LIST (rlConst, RLConst, CAR_RESPAWN_LOCATIONS_DROPSHOT);
		ATTR_LIST (rlConst, RLConst, CAR_RESPAWN_LOCATIONS_GRIDIRON);
