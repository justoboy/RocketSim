...
PyMemberDef BallState::Members[] = {
    ...
    {.name      = "attached_car_id",
        .type   = TypeHelper<decltype (RocketSim::BallState::AttachInfo::attachedCarId)>::type,
        .offset = offsetof (BallState, state) + offsetof (RocketSim::BallState, attachInfo) +
                  offsetof (RocketSim::BallState::AttachInfo, attachedCarId),
        .flags = 0,
        .doc   = "Attach: id of car the ball is welded to (0 = free)"},
    {.name      = "last_carrier_id",
        .type   = TypeHelper<decltype (RocketSim::BallState::AttachInfo::lastCarrierId)>::type,
        .offset = offsetof (BallState, state) + offsetof (RocketSim::BallState, attachInfo) +
                  offsetof (RocketSim::BallState::AttachInfo, lastCarrierId),
        .flags = 0,
        .doc   = "Attach: id of the car that most recently carried the ball"},
    {.name      = "engage_timer",
        .type   = TypeHelper<decltype (RocketSim::BallState::AttachInfo::engageTimer)>::type,
        .offset = offsetof (BallState, state) + offsetof (RocketSim::BallState, attachInfo) +
                  offsetof (RocketSim::BallState::AttachInfo, engageTimer),
        .flags = 0,
        .doc   = "Attach: seconds the ball has been attached to the current carrier"},
    {.name      = "release_cooldown",
        .type   = TypeHelper<decltype (RocketSim::BallState::AttachInfo::releaseCooldown)>::type,
        .offset = offsetof (BallState, state) + offsetof (RocketSim::BallState, attachInfo) +
                  offsetof (RocketSim::BallState::AttachInfo, releaseCooldown),
        .flags = 0,
        .doc   = "Attach: cooldown (seconds) before the ball can re-attach to the same car"},
    {.name      = "attach_active",
        .type   = TypeHelper<decltype (RocketSim::BallState::AttachInfo::active)>::type,
        .offset = offsetof (BallState, state) + offsetof (RocketSim::BallState, attachInfo) +
                  offsetof (RocketSim::BallState::AttachInfo, active),
        .flags = 0,
        .doc   = "Attach: true once the mode's attach mechanic is live"},
    {.name      = "dropshot_charge_level",
        .type   = TypeHelper<decltype (RocketSim::BallState::DropshotInfo::chargeLevel)>::type,
        .offset = offsetof (BallState, state) + offsetof (RocketSim::BallState, dsInfo) +
                  offsetof (RocketSim::BallState::DropshotInfo, chargeLevel),
        .flags = 0,
        .doc   = "Dropshot: charge level (1/2/3) controlling tile damage radius"},
    {.name      = "dropshot_accumulated_hit_force",
        .type   = TypeHelper<decltype (RocketSim::BallState::DropshotInfo::accumulatedHitForce)>::type,
        .offset = offsetof (BallState, state) + offsetof (RocketSim::BallState, dsInfo) +
                  offsetof (RocketSim::BallState::DropshotInfo, accumulatedHitForce),
        .flags = 0,
        .doc   = "Dropshot: accumulated hit force toward charge/supercharge"},
    {.name      = "dropshot_y_target_dir",
        .type   = TypeHelper<decltype (RocketSim::BallState::DropshotInfo::yTargetDir)>::type,
        .offset = offsetof (BallState, state) + offsetof (RocketSim::BallState, dsInfo) +
                  offsetof (RocketSim::BallState::DropshotInfo, yTargetDir),
        .flags = 0,
        .doc   = "Dropshot: which side of the field the ball can damage (0/-1/1)"},
    {.name      = "dropshot_has_damaged",
        .type   = TypeHelper<decltype (RocketSim::BallState::DropshotInfo::hasDamaged)>::type,
        .offset = offsetof (BallState, state) + offsetof (RocketSim::BallState, dsInfo) +
                  offsetof (RocketSim::BallState::DropshotInfo, hasDamaged),
        .flags = 0,
        .doc   = "Dropshot: whether a tile has been damaged"},
    {.name      = "dropshot_last_damage_tick",
        .type   = TypeHelper<decltype (RocketSim::BallState::DropshotInfo::lastDamageTick)>::type,
        .offset = offsetof (BallState, state) + offsetof (RocketSim::BallState, dsInfo) +
                  offsetof (RocketSim::BallState::DropshotInfo, lastDamageTick),
        .flags = 0,
        .doc   = "Dropshot: tick of the last tile damage"},
...
PyGetSetDef BallState::GetSet[] = {
    ...
    GETSET_ENTRY (BallState, local_offset, "Attach: ball offset in carrier local space"),
...
__init__(self,
	...
	attached_car_id: int = 0,
	last_carrier_id: int = 0,
	local_offset: RocketSim.Vec = RocketSim.Vec(),
	engage_timer: float = 0.0,
	release_cooldown: float = 0.0,
	attach_active: bool = False,
	dropshot_charge_level: int = 1,
	dropshot_accumulated_hit_force: float = 0.0,
	dropshot_y_target_dir: float = 0.0,
	dropshot_has_damaged: bool = False,
	dropshot_last_damage_tick: int = 0))"},
    {0, nullptr},
};
...
bool BallState::InitFromBallState (BallState *const self_, RocketSim::BallState const &state_) noexcept
{
	...
	auto localOffset = Vec::NewFromVec (state_.attachInfo.localOffset);

	if (!pos || !rotMat || !vel || !angVel || !localOffset)
		return false;
	...
	PyRef<Vec>::assign (self_->localOffset, localOffset.borrowObject ());
...
RocketSim::BallState BallState::ToBallState (BallState *self_) noexcept
{
	...
	state.attachInfo.localOffset = Vec::ToVec (self_->localOffset);
...
int BallState::Init (BallState *self_, PyObject *args_, PyObject *kwds_) noexcept
{
	...
	static char attachedCarIDKwd[]          = "attached_car_id";
	static char lastCarrierIDKwd[]          = "last_carrier_id";
	static char localOffsetKwd[]            = "local_offset";
	static char engageTimerKwd[]            = "engage_timer";
	static char releaseCooldownKwd[]        = "release_cooldown";
	static char attachActiveKwd[]           = "attach_active";
	static char dsChargeLevelKwd[]          = "dropshot_charge_level";
	static char dsAccumHitForceKwd[]        = "dropshot_accumulated_hit_force";
	static char dsYTargetDirKwd[]           = "dropshot_y_target_dir";
	static char dsHasDamagedKwd[]           = "dropshot_has_damaged";
	static char dsLastDamageTickKwd[]       = "dropshot_last_damage_tick";
	...
	PyObject *localOffset = nullptr;

	unsigned long carId                     = state.lastHitCarID;
	unsigned long long tickCountSinceUpdate = state.tickCountSinceUpdate;
	unsigned long attachedCarId             = state.attachInfo.attachedCarId;
	unsigned long lastCarrierId             = state.attachInfo.lastCarrierId;
	int attachActive                        = state.attachInfo.active;
	int dsChargeLevel                       = state.dsInfo.chargeLevel;
	int dsHasDamaged                        = state.dsInfo.hasDamaged;
	unsigned long long dsLastDamageTick     = state.dsInfo.lastDamageTick;

	if (!PyArg_ParseTupleAndKeywords (args_,
	        kwds_,
	        "|O!O!O!O!fffkK$KkkOOffpiffK",
	        dict,
	        Vec::Type,
	        &pos,
	        RotMat::Type,
	        ...
	        Vec::Type,
	        &localOffset,
	        &state.attachInfo.engageTimer,
	        &state.attachInfo.releaseCooldown,
	        &attachActive,
	        &dsChargeLevel,
	        &state.dsInfo.accumulatedHitForce,
	        &state.dsInfo.yTargetDir,
	        &dsHasDamaged,
	        &dsLastDamageTick))
		return -1;
	...
	if (localOffset)
		state.attachInfo.localOffset = Vec::ToVec (PyCast<Vec> (localOffset));

	state.lastHitCarID             = carId;
	state.tickCountSinceUpdate     = tickCountSinceUpdate;
	state.attachInfo.attachedCarId = attachedCarId;
	state.attachInfo.lastCarrierId = lastCarrierId;
	state.attachInfo.active        = attachActive;
	state.dsInfo.chargeLevel       = dsChargeLevel;
	state.dsInfo.hasDamaged        = dsHasDamaged;
	state.dsInfo.lastDamageTick    = dsLastDamageTick;
