...
PyMemberDef CarControls::Members[] = {
    ...
    {.name      = "handbrake",
        ...
        .doc    = "Handbrake"},
    {.name      = "powerup",
        .type   = TypeHelper<decltype (RocketSim::CarControls::powerup)>::type,
        .offset = offsetof (CarControls, controls) + offsetof (RocketSim::CarControls, powerup),
        .flags  = 0,
        .doc    = "Rumble powerup button (Spike Rush ball release)"},
...
int CarControls::Init (CarControls *self_, PyObject *args_, PyObject *kwds_) noexcept
{
	...
	static char powerupKwd[]   = "powerup";
	static char useItemKwd[]   = "use_item"; // for future use

	static char *dict[] = {
	    throttleKwd, steerKwd, pitchKwd, yawKwd, rollKwd, boostKwd, jumpKwd, handbrakeKwd, powerupKwd, useItemKwd, nullptr};

	RocketSim::CarControls controls{};
	int boost     = controls.boost;
	int jump      = controls.jump;
	int handbrake = controls.handbrake;
	int powerup   = controls.powerup;
	int useItem   = false;
...
PyObject *CarControls::Pickle (CarControls *self_) noexcept
{
	...
	if (controls.powerup != model.powerup &&
	    !DictSetValue (dict.borrow (), "powerup", PyBool_FromLong (controls.powerup)))
		return nullptr;
